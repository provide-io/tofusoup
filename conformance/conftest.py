...
from tofusoup.harness.logic import ensure_go_harness_build, TofuSoupError, GO_HARNESS_CONFIG
...
@pytest.fixture(scope="session")
def go_harness_executable(request, project_root: Path, loaded_tofusoup_config: dict) -> Path:
    """
    ...
    try:
        ...
        if not executable_path.exists() or not os.access(executable_path, os.X_OK):
            pytest.fail(f"Go harness executable '{harness_key}' missing or not executable at: {executable_path}")
...
def _ensure_openssl_available_if_needed_for_anything_else():
    if not shutil.which("openssl"):
        print("Warning: OpenSSL command line tool not found. Some tests or features might be affected if they rely on it directly.")
