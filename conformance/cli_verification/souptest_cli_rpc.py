...
def test_rpc_kv_server_help(soup_go_path: Path):
    """Verify 'soup-go rpc kv server --help' works and shows expected flags."""
    result = subprocess.run(
        [str(soup_go_path), "rpc", "kv", "server", "--help"],
        capture_output=True,
        text=True,
        timeout=5
    )
...
def test_rpc_kv_put_help(soup_go_path: Path):
    """Verify 'soup-go rpc kv put --help' works and shows expected flags."""
    result = subprocess.run(
        [str(soup_go_path), "rpc", "kv", "put", "--help"],
        capture_output=True,
        text=True,
        timeout=5
    )
...
def test_rpc_kv_get_help(soup_go_path: Path):
    """Verify 'soup-go rpc kv get --help' works and shows expected flags."""
    result = subprocess.run(
        [str(soup_go_path), "rpc", "kv", "get", "--help"],
        capture_output=True,
        text=True,
        timeout=5
    )
...
def test_rpc_kv_get_rejects_invalid_flags(soup_go_path: Path):
...
def test_rpc_kv_server_flag_validation(soup_go_path: Path):
...
def test_rpc_kv_client_flag_validation(soup_go_path: Path):
