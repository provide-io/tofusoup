...
from textual.widgets import ListView, Static, ListItem, DataTable
...
def search_results() -> list[SearchResult]:
    return [
        SearchResult(id="1", name="Module One", type="module", namespace="ns", provider_name="aws", registry_source="tf", total_versions=5, latest_version="1.0.0"),
        SearchResult(id="2", name="Module Two", type="module", namespace="ns", provider_name="gcp", registry_source="tofu", total_versions=10, latest_version="2.0.0"),
    ]
...
async def test_search_view_result_selection(
    pilot_with_message_capture: Callable[[list[Message]], Awaitable[Pilot]],
    ...
    result_selected_messages = [
        msg for msg in messages if isinstance(msg, SearchView.ResultSelected)
    ]
