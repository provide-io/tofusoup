...
1. Start the appropriate server (Go or Python) with the specified crypto configuration.
1. Create a client (Go or Python) configured to connect to the server.
1. Perform `PUT`, `GET`, and `DELETE` operations, verifying the correctness of each.
1. Test error handling, such as getting a non-existent key.
