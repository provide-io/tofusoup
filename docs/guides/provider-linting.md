...
1. OpenTofu experimental lint validation runs a normal HCL fixture through `tofu init` and `tofu validate -lint`.
...
`provider.source` and `provider.version` identify the package layout TofuSoup builds in an isolated filesystem mirror. `provider.environment` is passed only to the test provider process. It is useful for provider-owned feature switches during an upstream transition, but it does not turn that switch into an OpenTofu protocol feature.
